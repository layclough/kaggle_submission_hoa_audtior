...
def get_wheel_url(pkg_name, version):
    ...
    # Priority 2: py3/py2 + any
    for w in wheels:
        filename = w["filename"].lower()
        if "py3-none-any" in filename or "py2.py3-none-any" in filename:
            return w["url"], w["filename"]

    # Priority 3: any macosx/arm64 wheel compatible with python 3.14 (like abi3 wheels or cp3x wheels where x <= 14)
    # E.g. cp38-abi3-macosx_11_0_arm64.whl is compatible with cp314
    for w in wheels:
        filename = w["filename"].lower()
        if "macosx" in filename and ("arm64" in filename or "universal2" in filename):
            # Check if Python version tag is compatible (e.g. cp38, cp39, cp310, cp311, cp312, cp313, cp314, py3)
            # abi3 is compatible with all versions starting from the tagged version.
            if "abi3" in filename or "py3" in filename:
                return w["url"], w["filename"]
            # Check if tag matches cp314
            if "cp314" in filename:
                return w["url"], w["filename"]
                
    # Fallback: Just return any wheel if we couldn't match (pure python packages might have different tags)
    if wheels:
        # Sort by filename length or choose the first one
        return wheels[0]["url"], wheels[0]["filename"]
        
    return None, None

def install_pkg(pkg_spec):
    pkg_name, version = pkg_spec.split("==")
    url, filename = get_wheel_url(pkg_name, version)
    if not url:
        print(f"No compatible wheel found for {pkg_spec}")
        return False
    ...
    print(f"Extracting {filename} to {SITE_PACKAGES}...")
    try:
        ...
        print(f"Failed to extract/install {filename}: {e}")
...
def main():
    ...
    print("\n" + "="*50)
    print(f"Installation finished. Installed: {success_count}/{len(MISSING_PACKAGES)}")
    if failed_pkgs:
        print(f"Failed packages: {failed_pkgs}")
